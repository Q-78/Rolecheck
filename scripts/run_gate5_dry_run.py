...
OUTPUT = ARTIFACT_ROOT / "gate-5-dry-run-v0.1"
MODEL = Path(
...
def timeout_handler(signum: int, frame: object) -> None:
    raise TimeoutError("Gate 5 role generation exceeded 180 seconds")
...
def main() -> None:
    if OUTPUT.exists():
        raise RuntimeError("Gate 5 dry-run artifact target already exists")
    source_manifest = json.loads((GATE3 / "pilot-14-manifest.json").read_text())
    audit_preexecution_payload(source_manifest)
    if len(source_manifest["tasks"]) != EXPECTED_TASKS:
        raise RuntimeError("Gate 5 requires exactly 14 frozen tasks")
    team = build_pilot_team()
    aggregator = DeterministicMajorityAggregator()
    role_order = list(team.execution_protocol.execution_order)
    if len(role_order) != 3:
        raise RuntimeError("Gate 5 requires exactly three frozen roles")
    plan = {
        "gate": 5,
        ...
        "supersedes_artifact_files_hash": "sha256:49c415c16dc1725e65d34f3edcf8bdde87b607ba5b249e66fd9def100b839e15",  # noqa: E501
    ...
    dump(OUTPUT / "dry-run-plan.json", plan)
    ...
    try:
        ...
        for task_index, task_payload in enumerate(source_manifest["tasks"]):
            ...
            for role_id in role_order:
                ...
                if generation_seconds >= GPU_BUDGET_SECONDS:
                    raise RuntimeError("Gate 5 GPU generation budget exhausted")
        ...
        dump(OUTPUT / "dry-run-result.json", result)
